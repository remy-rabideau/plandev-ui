...
<script lang="ts">
  ...
  import DynamicFilterComponent from './DynamicFilter.svelte';
  import FilterTypeResult from './FilterTypeResult.svelte';

  // Generic filter type that works for both Activity and ExternalEvent
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  type GenericDynamicFilter = DynamicFilter<any>;

  type Filter = {
    dynamic_type_filters?: GenericDynamicFilter[];
    other_filters?: GenericDynamicFilter[];
    static_types?: string[];
    type_subfilters?: Record<string, GenericDynamicFilter[]>;
  };

  type TypeItem = { name: string };

  // DynamicFilterSchema is a flexible record type for the DynamicFilter component
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  type DynamicFilterSchema = Record<string, any>;
  ...
  // Mode configuration
  export let mode: 'activity' | 'externalEvent';
  export let title: string;
  export let allTypes: TypeItem[] = [];
  export let matchingTypes: TypeItem[] = [];
  export let instanceCount: number = 0;

  // Icons for the resulting types section
  export let TypeIcon: ComponentType<SvelteComponent>;
  export let InstanceIcon: ComponentType<SvelteComponent>;

  // Schema configuration
  export let dynamicTypeFilterSchema: DynamicFilterSchema;
  export let otherFilterSchema: DynamicFilterSchema;
  export let typeSubfilterSchema: DynamicFilterSchema;

  // Hints for the filter sections
  export let dynamicTypeFilterHint: string = 'Type includes...';
  export let otherFilterHint: string = 'Names, attributes...';
  export let noItemsMessage: string = 'No items matching your filter';

  // Default field when adding new filters
  export let defaultDynamicTypeField: string = 'Type';
  export let defaultOtherFilterField: string = 'Name';

  let dirtyFilter: Filter = {
  ...
  export function setActiveFilter(newFilter: Filter) {
  ...
  function onAddAllManualTypes() {
    dirtyFilter = { ...dirtyFilter, static_types: filteredTypes.map(t => t.name) };
    dispatch('filterChange', { filter: dirtyFilter });
  }
  ...
  function onAddDynamicFilter(list: 'dynamic_type_filters' | 'other_filters') {
    const field = list === 'dynamic_type_filters' ? defaultDynamicTypeField : defaultOtherFilterField;
    const listObj = dirtyFilter[list] || [];
    const currentFilters = Array.isArray(listObj) ? listObj : [];
    const id = getNextThingID(listObj);
    dirtyFilter = {
      ...dirtyFilter,
      [list]: [...currentFilters, { field, id, operator: 'includes', value: '' } as GenericDynamicFilter],
    };
    dispatch('filterChange', { filter: dirtyFilter });
  }

  function onDynamicFilterChange(
    list: 'dynamic_type_filters' | 'other_filters',
    { detail: { filter: changedFilter } }: CustomEvent,
  ) {
    const currentFilters = Array.isArray(dirtyFilter[list]) ? dirtyFilter[list] : [];
    dirtyFilter = {
      ...dirtyFilter,
      [list]: (currentFilters || []).map(f => {
        if (f.id === changedFilter.id) {
  ...
  function onDynamicFilterRemove(list: 'dynamic_type_filters' | 'other_filters', id: number) {
    const currentFilters: Filter['dynamic_type_filters'] | Filter['other_filters'] = Array.isArray(dirtyFilter[list])
      ? dirtyFilter[list]
      : [];
    dirtyFilter = {
      ...dirtyFilter,
  ...
  function onTypeSubfilterChange(type: string, { detail: { filter: changedFilter } }: CustomEvent) {
  ...
  function onAddTypeSubfilter(type: string) {
    ...
    dirtyFilter = {
      ...
      type_subfilters: {
        ...typeSubfilters,
        [type]: [...currentFilters, { field: 'Name', id, operator: 'includes', value: '' } as GenericDynamicFilter],
      },
    };
    dispatch('filterChange', { filter: dirtyFilter });
  }
  ...
  $: activityOrEvent = mode === 'activity' ? 'activity' : ('event' as 'activity' | 'event');
</script>
...
<div bind:this={rootRef} class="w-full" style:display="grid">
  ...
  {#if shown}
    <Draggable
      className="st-menu filter-builder"
    ...
    >
      ...
      <div class="body">
        <CssGrid columns="0.7fr 3px 0.3fr" columnMinSizes={{ 0: 500, 1: 3, 2: 300 }} class="filter-builder-grid">
          <div class="filters">
                  ...
                  >
                    <div class="manual-types-menu">
                      {#if filteredTypes.length > 0}
                        ...
                        {/each}
                      {:else}
                        <MenuItem disabled>{noItemsMessage}</MenuItem>
                      {/if}
                    </div>
                  </Menu>
                </div>
                {#if dirtyFilter.static_types?.length}
                  <div class="manual-types-results">
                    {#each dirtyFilter.static_types as name}
                      <FilterTypeResult {name} {activityOrEvent} on:remove={() => onManualTypeToggled(name)} />
                    {/each}
                  </div>
                {/if}
              </div>
            ...
            <div class="filter-section" aria-label="dynamic-types">
              <div class="filter-section-header st-typography-medium">
                <div class="filter-section-title">
                  ...
                  <div class="hint st-typography-body">{dynamicTypeFilterHint}</div>
                </div>
                <button
                  class="st-button icon"
              ...
              {#if dirtyFilter.dynamic_type_filters?.length}
                <div class="filter-section-content">
                  <div class="dynamic-filter-content" role="list">
                    {#each dirtyFilter.dynamic_type_filters as filterItem, i (filterItem.id)}
                      <DynamicFilterComponent
                        filter={filterItem}
                        on:remove={() => onDynamicFilterRemove('dynamic_type_filters', filterItem.id)}
                        on:change={event => onDynamicFilterChange('dynamic_type_filters', event)}
                        verb={i === 0 ? 'Where' : 'and'}
                        schema={dynamicTypeFilterSchema}
                      />
                    {/each}
                  </div>
                </div>
            ...
            <div class="filter-section" aria-label="other-filters">
              <div class="filter-section-header st-typography-medium">
                <div class="filter-section-title">
                  Other Filters
                  <div class="hint st-typography-body">{otherFilterHint}</div>
                </div>
                <button
                  class="st-button icon"
                  aria-label="Add Filter"
              ...
              {#if dirtyFilter.other_filters?.length}
                <div class="filter-section-content">
                  <div class="dynamic-filter-content" role="list">
                    {#each dirtyFilter.other_filters as filterItem, i (filterItem.id)}
                      <DynamicFilterComponent
                        filter={filterItem}
                        on:remove={() => onDynamicFilterRemove('other_filters', filterItem.id)}
                        on:change={event => onDynamicFilterChange('other_filters', event)}
                        verb={i === 0 ? 'Where' : 'and'}
                        schema={otherFilterSchema}
                      />
                    {/each}
                  </div>
                </div>
          ...
          <div class="resulting-types">
            ...
            <div class="resulting-types-list">
              {#each filteredMatchingTypes as type}
                <FilterTypeResult name={type.name} {activityOrEvent} removable={false}>
                  <button
                    slot="right"
                  ...
                  <svelte:fragment slot="bottom">
                    {#if dirtyFilter.type_subfilters && dirtyFilter.type_subfilters[type.name] && dirtyFilter.type_subfilters[type.name].length}
                      <div class="resulting-type-filters">
                        {#each dirtyFilter.type_subfilters[type.name] as subfilter (subfilter.id)}
                          <DynamicFilterComponent
                            filter={subfilter}
                            on:remove={() => onTypeSubfilterRemove(type.name, subfilter.id)}
                            on:change={event => onTypeSubfilterChange(type.name, event)}
                            verb={''}
                            schema={typeSubfilterSchema}
                          />
                        {/each}
                      </div>
                    {/if}
